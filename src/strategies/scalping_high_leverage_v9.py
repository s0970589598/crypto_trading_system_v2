"""
高槓桿剝頭皮策略 v9 (Scalping High Leverage)
專為 25x-125x 槓桿設計

v9 更新（環境自適應版）：
目標：解決 60 天回測中的週勝率波動和極端虧損問題

核心改進（基於 v8）：
1. 波動率環境分類 - 高/中/低波動環境使用不同參數
2. 連虧保護機制 - 連續虧損後降低風險敞口
3. 時段過濾 - 避開低流動性時段
4. 動態止損寬度 - 根據環境調整 ATR 倍數
...
class ScalpingHighLeverage:
    ...
    def __init__(
        ...
        regime_lookback: int = 60,           # 60 分鐘計算環境
        low_vol_threshold: float = 0.7,      # ATR ratio < 0.7 = 低波動
        high_vol_threshold: float = 1.3,     # ATR ratio > 1.3 = 高波動
        
        # 環境自適應止損
        low_vol_atr_mult: float = 1.2,       # 低波動：更緊止損
        mid_vol_atr_mult: float = 1.5,       # 中波動：標準止損
        high_vol_atr_mult: float = 1.8,      # 高波動：更寬止損
        ...
        max_consecutive_losses: int = 2,     # 連續 2 虧後暫停
        cooldown_bars: int = 60,             # 暫停 60 分鐘
        
        # 時段過濾（UTC 時間）
        use_session_filter: bool = True,
        avoid_low_liquidity: bool = True,    # 避開低流動性時段
        # 低流動性時段：UTC 21:00-01:00（亞洲凌晨）
        ...
        self.name = "Scalping High Leverage v9"
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.use_session_filter and self.avoid_low_liquidity:
            ...
            if self.low_liquidity_start > self.low_liquidity_end:
                # 跨午夜（如 21:00 - 01:00）
                df['low_liquidity'] = (
                    (df['hour'] >= self.low_liquidity_start) | 
                    (df['hour'] < self.low_liquidity_end)
                )
            else:
                df['low_liquidity'] = (
                    (df['hour'] >= self.low_liquidity_start) & 
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # v8: 只使用強信號 + v9: 時段過濾 + v9.1: 環境過濾
        if self.use_strong_signals_only:
            long_condition = combo_a_long & uptrend & trend_strong & adx_ok & volatility_ok & realtime_vol_ok & momentum_ok_long & bb_width_ok & volume_ok & session_ok & regime_ok
        else:
            long_condition = (combo_a_long | combo_b_long) & uptrend & trend_strong & adx_ok & volatility_ok & realtime_vol_ok & momentum_ok_long & bb_width_ok & volume_ok & session_ok & regime_ok
        ...
        # v8: 只使用強信號 + v9: 時段過濾 + v9.1: 環境過濾
        if self.use_strong_signals_only:
            short_condition = combo_a_short & downtrend & trend_strong & adx_ok & volatility_ok & realtime_vol_ok & momentum_ok_short & bb_width_ok & volume_ok & session_ok & regime_ok
        else:
            short_condition = (combo_a_short | combo_b_short) & downtrend & trend_strong & adx_ok & volatility_ok & realtime_vol_ok & momentum_ok_short & bb_width_ok & volume_ok & session_ok & regime_ok
        
        # 設置信號
        df.loc[long_condition, 'long_signal'] = True
        df.loc[short_condition, 'short_signal'] = True
        
        # === 計算止損止盈（v9: 使用動態 ATR 倍數）===
        if self.use_regime_filter:
            atr_stop = df['atr'] * df['dynamic_atr_mult']
        else:
...
def quick_test():
    """快速測試"""
    print("=" * 60)
    print("高槓桿剝頭皮策略 v9 (Scalping High Leverage)")
    print("=" * 60)
    print("\n策略特點：")
    ...
    print("\nv9 優化（環境自適應）：")
    print("• 波動率環境分類：低/中/高波動使用不同止損")
    print("• 動態 ATR 倍數：低波動 1.2x，中波動 1.5x，高波動 1.8x")
    print("• 時段過濾：避開 UTC 21:00-01:00 低流動性時段")
    print("• 連虧保護：連續 2 虧後暫停 60 分鐘（回測引擎支援）")
    print("\n適用場景：")
    print("• 槓桿：25x - 125x")
    print("• 時間框架：1 分鐘")
    print("• 持倉時間：1-15 分鐘")
    print("\n使用方法：")
    print("./venv/bin/python tools/backtest.py --strategy scalping --days 60 --timeframe 1m --leverage 50")
    print("./venv/bin/python -c \"from tools.data_loader import DataLoader; DataLoader().load_from_csv('data/BTC_USDT_1m_60days.csv')\"")
    print("=" * 60)
